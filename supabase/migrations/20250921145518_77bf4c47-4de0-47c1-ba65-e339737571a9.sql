...
-- Fix search_path for functions (re-create with proper search_path)
CREATE OR REPLACE FUNCTION public.log_book_interaction(
  p_user_id text, 
  p_book_title text, 
  p_book_author text, 
  p_book_isbn text DEFAULT NULL::text, 
  p_interaction_type text DEFAULT 'view'::text, 
  p_digital_source text DEFAULT NULL::text, 
  p_source_context text DEFAULT NULL::text, 
  p_device_type text DEFAULT NULL::text, 
  p_browser_type text DEFAULT NULL::text, 
