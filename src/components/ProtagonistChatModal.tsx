        ...
        {mode === 'mission' && missionPhase !== 'complete' && (
          <div className="border-t border-slate-700/50">
            ...
            <input
              ...
              className="flex-1 h-11 bg-slate-800 border border-slate-700/50 rounded-lg px-3 py-0 text-sm text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-violet-500/50"
              disabled={isLoading}
            />
            <button
              onClick={() => sendMissionAction(missionInput)}
              disabled={!missionInput.trim() || isLoading}
              className="h-11 w-11 flex items-center justify-center bg-violet-500/20 border border-violet-500/30 text-violet-400 rounded-lg hover:bg-violet-500/30 disabled:opacity-30 transition-all flex-shrink-0"
              >
                <Send className="w-4 h-4" />
              </button>
            </div>
