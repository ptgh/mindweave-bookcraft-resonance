    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        ...
        <main id="main-content" className="container mx-auto px-4 md:px-6 py-8 md:py-12">
          ...
          <div className="max-w-md mx-auto mb-8 flex gap-2 items-stretch">
            <div className="relative flex-1 min-w-0">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <Input
                value={search}
                onChange={e => setSearch(e.target.value)}
                placeholder="Search by title, author, or cha…"
                className="pl-10 h-10 bg-slate-800/50 border-slate-600 text-slate-200 placeholder-slate-400 focus:border-violet-400 truncate"
              />
            </div>
            <button
              onClick={() => setViewMode(v => v === 'books' ? 'portraits' : 'books')}
              className={`flex items-center gap-1.5 px-3 h-10 rounded-md border text-xs font-medium transition-all whitespace-nowrap ${
                viewMode === 'portraits'
