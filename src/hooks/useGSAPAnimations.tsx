...
export const useGSAPAnimations = () => {
  ...
  // Function to add feature block refs - memoized to prevent re-renders
  const addFeatureBlockRef = useCallback((el: HTMLDivElement | null) => {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      // Safety check - don't create context if component already unmounted
      if (!mainContainerRef.current && !heroTitleRef.current && featureBlocksRef.current.length === 0) {
        return;
      }
    ...
    return () => {
      ...
      if (contextRef.current) {
        try {
          ...
          console.warn('GSAP context cleanup failed:', error);
        }
      ...
      // Reset feature blocks ref for next mount
      featureBlocksRef.current = [];
    };
  }, []);
